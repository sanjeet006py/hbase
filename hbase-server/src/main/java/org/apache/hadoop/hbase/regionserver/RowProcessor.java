 ...
 * invoked by {@link Region#processRowsWithLocks(RowProcessor)}. This class performs scans and
...
public interface RowProcessor<S extends Message, T extends Message> {
  ...
  /**
   * Rows to lock while operation. They have to be sorted with <code>RowProcessor</code> to avoid
   * deadlock.
   */
  Collection<byte[]> getRowsToLock();

  /**
   * Obtain the processing result. All row processor implementations must implement this, even if
   * the method is simply returning an empty Message.
   */
  T getResult();
  ...
  /**
   * HRegion handles the locks and MVCC and invokes this method properly. You should override this
   * to create your own RowProcessor. If you are doing read-modify-write here, you should consider
   * using <code>IsolationLevel.READ_UNCOMMITTED</code> for scan because we advance MVCC after
   * releasing the locks for optimization purpose.
