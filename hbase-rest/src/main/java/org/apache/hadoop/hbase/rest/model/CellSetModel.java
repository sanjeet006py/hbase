  ...
  public Message messageFromObject() {
    ...
    for (RowModel row : getRows()) {
      ...
      if (row.getKeyLength() == MAGIC_LENGTH) {
        ...
        rowBuilder
          .setKey(UnsafeByteOperations.unsafeWrap(row.getKeyArray(), row.getKeyOffset(), row.getKeyLength()));
      }
      for (CellModel cell : row.getCells()) {
        ...
        if (cell.getValueLength() == MAGIC_LENGTH) {
          ...
          cellBuilder.setData(
            UnsafeByteOperations.unsafeWrap(cell.getValueArray(), cell.getValueOffset(), cell.getValueLength()));
        }
