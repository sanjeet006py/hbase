  ...
  protected void takeSnapshot(SnapshotDescription snapshot)
    ...
    if (!isDone) {
      ...
      if (snapshotManager.isTakingSnapshot(getTableName())) {
        ...
        // prepareToTakeSnapshot does the snapshot preflight checks and prepares the working
        // directory for the snapshot. We must call this first.
        snapshotManager.prepareToTakeSnapshot(snapshot);
        ...
        if (cpHost != null) {
          ...
          desc = masterServices.getTableDescriptors().get(getTableName());
